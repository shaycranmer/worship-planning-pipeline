...
This pipeline turns that knowledge into structured data, automates the mechanical steps, and — this is the design's spine — **flags rather than decides**. Every judgment call is surfaced to the human, with the evidence attached.
...
| 3 | **Scripture retrieval** — full NRSV text fetched from the source at build time; scripture is never re-typed and never generated | `get_scripture.py` |
...
The build reads `worship_input.json`, ingests the liturgist's blocks from `handoff/2026-07-19/`, fetches the scripture, runs the flag pass, and writes a formatted service plan to `Worship Plans/` plus a `builder_flags.md` beside the handoff. (Scripture fetching needs a network connection; everything else is local.)
...
get_scripture.py           NRSV retrieval — scripture never touches a model
